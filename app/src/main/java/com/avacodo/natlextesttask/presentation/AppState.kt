...
sealed class AppState<ResultType> {
    abstract fun handleState(
        onInitializationAction: (result: ResultType) -> Unit,
        onStartLoadingAction: () -> Unit,
        onSuccessAction: (result: ResultType) -> Unit,
        onErrorAction: (message: String) -> Unit,
    )

    class Initial<ResultType>(private val result: ResultType) : AppState<ResultType>(){
        override fun handleState(
            ...
            onInitializationAction.invoke(result)
        }
    }
